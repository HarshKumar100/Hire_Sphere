...
import { IoArrowBack, IoAdd, IoClose, IoSave } from 'react-icons/io5';
...
const EditDrive = () => {
    ...
    const fetchDrive = async () => {
        try {
            ...
            if (response.success && response.data.drive) {
                ...
                setFormData({
                    ...
                    ctcMin: drive.ctc?.min?.toString() || '',
                    ctcMax: drive.ctc?.max?.toString() || '',
                    positions: drive.positions?.toString() || '',
                    applicationDeadline: drive.applicationDeadline ? drive.applicationDeadline.split('T')[0] : '',
                    driveDate: drive.driveDate ? drive.driveDate.split('T')[0] : '',
                    minCGPA: drive.eligibilityCriteria?.minCGPA?.toString() || '',
                    maxBacklogs: drive.eligibilityCriteria?.maxBacklogs?.toString() || '0',
                    minTenthMarks: drive.eligibilityCriteria?.minTenthMarks?.toString() || '',
                    minTwelfthMarks: drive.eligibilityCriteria?.minTwelfthMarks?.toString() || '',
                    branches: drive.eligibilityCriteria?.branches || [],
                    allowedBatches: drive.eligibilityCriteria?.allowedBatches || [],
                    skillsRequired: drive.skillsRequired || [],
                });

                setSelectionStages(drive.selectionProcess || [{ stage: '', description: '' }]);
            }
        } catch (error) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await driveService.updateDrive(id, driveData);
            if (response.success) {
        ...
        <div className="space-y-6">
            <FadeIn>
                <div className="flex items-center space-x-4">
                    ...
                    <div>
                        <h1 className="text-3xl font-bold text-primary-900">Edit Drive</h1>
                        <p className="text-primary-600 mt-1">Update your placement drive details</p>
                    </div>
                </div>
            </FadeIn>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <FadeIn delay={0.6}>
                    <div className="flex justify-end space-x-4">
                        ...
                        <Button type="submit" loading={saving} disabled={saving} icon={<IoSave />}>
                            Save Changes
                        </Button>
                    </div>
                </FadeIn>
